...
class StakingContract extends Contract {
  ...
  onCall(functionName: string, params: Uint8Array): ErrorCode {
    ...
    } else if (functionName === 'GetStakingInfo') {
      return this.getStakingInfo(params);
    }
  ...
  private stake(params: Uint8Array): ErrorCode {
    ...
    const caller = Context.getCaller();
    const result = Staking.stake(caller, validator, amount, this.tokenID);
  ...
  private unstake(params: Uint8Array): ErrorCode {
    ...
    //   锁定期、解质押冷却期等应在应用层实现
    const caller = Context.getCaller();
    const result = Staking.unstake(caller, validator, amount, this.tokenID);
    if (result !== ErrorCode.SUCCESS) {
      return result;
    }
  ...
  private delegate(params: Uint8Array): ErrorCode {
    ...
    // 步骤3：使用 SDK 基础能力进行委托
    const caller = Context.getCaller();
    const result = Staking.delegate(caller, validator, amount, this.tokenID);
    if (result !== ErrorCode.SUCCESS) {
      return result;
    }
  ...
  private undelegate(params: Uint8Array): ErrorCode {
    ...
    const caller = Context.getCaller();
    const result = Staking.undelegate(caller, validator, amount, this.tokenID);
    if (result !== ErrorCode.SUCCESS) {
      return result;
    }
   ...
   * 查询指定地址的质押信息
   */
  private getStakingInfo(params: Uint8Array): ErrorCode {
    const paramsStr = String.UTF8.decode(params.buffer);
...
export function Execute(): u32 {
  const maxLen = 8192;
  // 简化：假设函数名通过其他方式传递
  const functionName = 'GetStakingInfo'; // 示例
  const params = HostABI.getContractInitParams(maxLen);
  if (params === null) {
    return ErrorCode.ERROR_INVALID_PARAMS;
  }
