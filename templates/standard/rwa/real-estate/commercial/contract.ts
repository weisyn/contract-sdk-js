...
class equityContract extends Contract {
  ...
  onCall(functionName: string, params: Uint8Array): ErrorCode {
    if (functionName === 'Tokenizeequity') {
      return this.tokenizeAsset(params);
    } else if (functionName === 'Transferequity') {
      return this.transferAsset(params);
    } else if (functionName === 'Escrowequity') {
...
const contract = new equityContract();
...
export function Execute(): u32 {
  const maxLen = 8192;
  const functionName = 'Tokenizeequity';
  const params = HostABI.getContractInitParams(maxLen);
  if (params === null) {
    return ErrorCode.ERROR_INVALID_PARAMS;
  }
