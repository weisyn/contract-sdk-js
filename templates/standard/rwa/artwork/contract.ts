...
class artworkContract extends Contract {
  ...
  onCall(functionName: string, params: Uint8Array): ErrorCode {
    if (functionName === 'Tokenizeartwork') {
      return this.tokenizeAsset(params);
    } else if (functionName === 'Transferartwork') {
      return this.transferAsset(params);
    } else if (functionName === 'Escrowartwork') {
...
const contract = new artworkContract();
...
export function Execute(): u32 {
  const maxLen = 8192;
  const functionName = 'Tokenizeartwork';
  const params = HostABI.getContractInitParams(maxLen);
  if (params === null) {
    return ErrorCode.ERROR_INVALID_PARAMS;
  }
